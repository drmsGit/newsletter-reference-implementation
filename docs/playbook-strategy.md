...
### 2026-07-31 — AI Layer Design (Clusters 1, 2, 5 → ADR-140 / ADR-141 / ADR-144)

**Context:** A forward-design interview (`docs/architecture/interview-prep/AI Layer - design interview.md` — 5 clusters, one per planned ADR) worked through *what* the AI layer is before writing it, building directly on the 2026-07-11 directional decisions (per-capability AI governance config; AI capabilities as pluggable extension points). Clusters 1 (foundational), 2 (Mode A in-app actions) and 5 (data/model/cost governance) are now settled and written up; Clusters 3 (Mode B autonomous workflows) and 4 (Mode C dev-time) remain open.

1. **The trust layer is reframed ([[ADR-140 — AI Capability Layer]]).** "AI proposes, human governs" is realized as **direct write by default — reversible + audited + a human can interfere — NOT "everything is a pending proposal."** Routing every AI action through the override queue would force it to evaluate *all* campaigns (token/performance cost) and bury managers in approvals. Approval is instead a **per-task setting** (auto-apply vs require-approval), enabling graduated trust. AI plugs in behind the existing seams (provider-adapter + plugin-registry), stays optional per capability, and always has a global kill switch. Prompts are **frontend-editable, content-style versioned, manager-owned** — the dev owns only a thin per-task scaffold; a global "DON'T EVER" guardrail file holds negative constraints. Audit logs the standard fields + the published prompt-version id.

2. **Mode A — in-app assistive actions ([[ADR-141 — In-App Assistive AI Actions]]).** A task-plugin contract (scaffold declares inputs / output / where-it-lands + references to the manager-owned prompt, model, guards, approval mode); build 2–3 tasks concretely, then extract the registry. First three, built 2 → 1 → 3: **subject/preheader** (easiest first win), **auto-tag** (ADR-080 propose-govern), **content-suggestion-with-reasons** (ADR-081 ranks-within-governed-sets). One reused accept/reject inbox (the same approval surface Mode B will use). AI output is an **unpublished draft**; publishing is the governed step. **Architectural guard:** AI generates a bounded set of *shared* variants, **never per-recipient content on the fly** — per-recipient personalization stays with the decision engine + merge variables, so the approval surface can't explode.

3. **Cross-cutting data / model / cost governance ([[ADR-144 — AI Data and Model Governance]]).** An `AIProvider` adapter (mirrors the send adapter; ship Claude + one EU worked example so "GDPR-friendly is possible" is demonstrated). **No residency warning flag** — we take no data-protection liability; the company chooses provider/residency, we default to minimization. **PII line:** per-task, default no raw PII to the model (personalize via merge variables, ADR-005), opt-up under a DPA and logged. **Cost cap:** warn → hard stop, per role/user, enforced as a **pre-call gate** — each task's worst-case total (`count_tokens(input)` + its output ceiling) is computed before running, and any task that wouldn't fit under the remaining cap is refused *before it starts* (so a near-limit stop lands between tasks, never mid-task; a running task that hits its own output ceiling still shows the partial). Buffer is **configurable and role/permission-bound**. Per-run estimate dropped from the UI. Phase-1 MVP stops here; an opt-in "keep going, pay per token" overage mode + richer controls = future "AI extra package".
...
### 2026-08-02 — Security Chapter, Part 1 (tenancy, access, brands) — interview OPEN

**Context:** The Security chapter was promoted from "parked" to next-up because it is the only named blocker on 4C (write & publish), and because user/role authentication has the widest blast radius left — cheaper to define before more surface is built. Worked as a design interview, one question at a time. **These are recorded decisions, not yet an ADR;** the interview is still open (see Status below).

1. **Single-tenant, definitively.** One install = one company. Several brands of the *same* company may share an install, but no agency runs multiple clients on one system — the agency helps a company host on the company's own servers and helps operate it. Consequence: no tenant filter on any query, and GDPR separation is a deployment boundary rather than a code one. Multi-tenancy is the one thing that cannot be retrofitted, so it is being ruled out deliberately rather than by omission.

2. **Three seeded roles over a permission model, not a role enum.** Admin (settings, users, keys, provider/AI config) · Manager (campaigns, content, audiences, sends, AI actions) · Viewer (read-only). The three ship as a **preset**; roles and permissions are rows, so a company can add roles or manage sub-level permissions without touching code — same convention-based-extension posture as the strategy and template plugins. Explicitly **not** modelling the average company's org chart: companies mostly run everything as admin, or bring their own scheme. Assignment is **(user × role × brand)** — one mechanism serving both role and brand scoping rather than two systems side by side. The **agency operator is not a role**: they are an external Admin or Manager, distinguished by audit accountability, not capability.

3. **Brands are a scope, not a hierarchy** — "like categories". A switcher in the top navigation; content and branding differ per brand, most everything else carries over. **The GDPR-separation case gets no option at all:** if a company must keep brand A's and brand B's people apart, they run *two installs*. Shipping a mixed-but-supposedly-separated mode would imply we had judged it compliant — exactly the liability [[ADR-144 — AI Data and Model Governance]] §3 refuses. A company that wants it anyway changes its own code. **The visual-only case must stay degenerate:** a company using brands purely as a skin needs only theme selection, so one implicit default brand exists and the machinery stays invisible until a second brand is created.

4. **Consent splits by purpose, not by brand.** The transactional question forces the restructure regardless — marketing opt-in and transactional basis are different permissions on the same person — so the axis is *purpose*, with brand carried alongside on the grant rather than partitioning the data. The **selectable audience derives from the permission table**. This absorbs a substantial part of the transactional-sends Needs-ADR item (flagged there as a possible adoption dealbreaker). **Known cost:** `RecipientDB.consent_status` is a single field today, with the consent gate built on it in `resolve_audience` and `execute_decision_slot` — this makes it a relation, a real change to shipped, working code, and far cheaper now than after brands exist.

5. **Signals are shared at person level; content candidates are scoped.** Brands are presentation, so engagement is engagement — and a recipient moving from brand A to brand B arrives with useful history rather than as a stranger. The accident to prevent is *content* leaking across brands via those shared signals, so candidates **default to the sending brand**. Safe by construction, with widening a deliberate and visible act — the same idiom the architecture already uses for the mock-provider default, ADR-144's PII line, and the governed model list.

6. **`recipient.brand` is the sending brand (navigation context), not an intrinsic person attribute.** A person connected to brands A and B "is" brand A in the context of a send from brand A. Brand affiliation may additionally arrive from the CRM as a projected attribute (consistent with [[ADR-120 — CRM as Customer Source of Truth]] / [[ADR-126 — Maintain Local Recipient Projection]]) feeding the permission table — a source of the data, not a competing concept. **Brand as a decision-strategy candidate filter** (own brand / a named list / all brands) is expressible through the existing `candidate_filter_fields` manifest, so it needs no new machinery. **Explicitly demonstrate, don't build:** not POC or standard-package scope, but the playbook must show *that* and *how* it is possible.

### 2026-08-02 — Security Chapter, Part 2 (authentication, secrets, audit, erasure)

7. **Passwordless authentication with a typed code — not a magic link.** Removes password storage entirely, and with it a whole class of Art. 32 risk. The code-over-link choice is deliberate: corporate mail scanners and Apple MPP **pre-fetch links**, consuming a single-use token before the human clicks — the same pre-fetch behaviour the signal layer already accounts for, here producing "this link has already been used" as a routine support case. Short TTL, single-use, rate-limited per address *and* per IP, and an identical response whether or not the address exists so the login form is not an account-enumeration oracle. Sessions rotate on login, carry absolute plus idle timeouts, and are server-side revocable.

8. **SSO/SCIM is documented, not built.** Passwordless solves *authentication*; SSO solves *lifecycle governance* — they are different problems, and only the second is what Mittelstand procurement actually asks about. For the standard package, offboarding is **admin deactivation plus a visible access list** (last login, external flag); SSO/SCIM is the well-documented upgrade path for companies needing automatic deprovisioning. This is a real residual risk accepted knowingly: nothing tells the system when an external agency operator leaves the agency, and a stale external Admin is the most valuable thing on the box — hence the access list is part of the decision, not decoration.

9. **Secrets: the environment is the interface; deployment decides how it is populated.** The application reads `os.environ` and never learns where a value came from, so the existing code pattern is already correct and the problem is documentation: `.env` is **dev-only**, production populates the environment via Docker/Compose secrets, systemd credentials or Kubernetes secrets, with a managed store (Vault, Infisical, Scaleway Secret Manager) as the documented seam. Rotation is "update the environment, restart" — true today by accident, now by design. **Credentials are write-only in the UI:** an Admin may set or replace a key and see that one exists, never read it back, and the API must never return one. This follows directly from the agency-operator decision — otherwise "the agency helps operate it" quietly means the agency can exfiltrate the company's sending and AI credentials.

10. **One append-only audit log for actor-attributed events, alongside the existing domain records.** The domain logs (`AIRunDB`, `ContentOverrideDB`, `ConsentSyncLogDB`, delivery history) are *history* — the product needs them to work. The audit log serves **accountability**: who asked, when, from where. The overlap is worth paying for: one screen for "what did this operator do", one export for a company's SIEM, and somewhere to put events that have no domain record at all (login success/failure, role assignment, deactivation, credential change). The actor must be able to be a **system or integration**, not only a person — ADR-142 already requires machine-initiated actions in the same history, so it is cheaper to build in now than to retrofit when Mode B lands. **Scope: writes, plus exports and bulk reads** — anything moving personal data out of the system or pulling it in volume — but **not individual page views**, which would drown the signal without catching a different risk.

11. **Erasure separates identity from activity.** Audit entries, delivery executions, signal contributions and engagement events reference the **internal recipient id** ([[ADR-054 — Use Internal Recipient Identifiers]] already set this up), never the email address. An Art. 17 erasure deletes or irreversibly anonymises the *identity* row while activity records keep an id that now points to nobody — accountability survives, the person is genuinely gone, and there is no tombstone that is secretly still personal data. **The enforcing rule: the audit log stores ids and never contact details** — one "sent to anna@example.com" in a payload field breaks the separation and turns erasure into rewriting history. Two consequences stated explicitly: **signals are personal data** (per-person behavioural profiles — erasure takes them too, and they live in a different table so this is easy to forget), and **rendered snapshots contain merged personal data**, which is the same question as the open snapshot-storage Needs-ADR item and another argument for reconstructing on demand rather than persisting per-recipient HTML. **One deliberate exception:** proof of consent is retained in minimised form (id, timestamp, source, what was granted) under Art. 17(3)(e), since the defence against a UWG §7 complaint is showing the opt-in.

**Status:** the architecturally consequential questions are settled. **Remaining are documentation rather than design** — breach response (Art. 33/34, 72h) is the adopter's process, where the architecture's job is to make detection possible (audit log) and scope determinable; and retention *periods* are the company's call, where the architecture supplies the prune mechanism (ties to the existing data-lifecycle Needs-ADR item). No ADR written yet — the above is the decision record.

### 2026-08-02 — Security Chapter, Part 3 (first slice built; system mail is a third stream)

12. **Forced login, with the code on screen.** Every UI page requires a signed-in user, applied as one router-level dependency in `main.py` rather than 57 route decorators — the reason the blast radius feared at the start did not materialise. The lockout objection is answered by the mail default rather than by weakening the rule: system mail defaults to the **mock** provider, so the sign-in code appears on screen and never depends on a mail path that might not work. Enforcement stays a **setting** so a misconfigured deployment has a way back that is not editing an environment variable on the server; it ships off, startup states which mode it is in, and it is turned on once a deployment has signed in successfully. **JSON API routers are deliberately not guarded** — that is *machine* authentication, separately scoped as a Mode B prerequisite, and a human session cookie would be the wrong mechanism.

13. **System mail is a third outbound stream, not a flavour of transactional.** *marketing customer / transactional customer / **system internal***. System mail goes to the company's own operators, so it carries no consent question, no unsubscribe semantics and almost no volume. What separates it from transactional is the **circular failure**: if transactional delivery degrades a customer misses a receipt, but if system delivery degrades nobody can sign in — including the person who would fix the cause. Transactional volume and complaint exposure are still customer-driven and outside the company's control, so sharing with it reintroduces the risk. Sign-in codes are only the first consumer; **approval notifications and AI-suggestion alerts belong on the same channel**, which is why the configuration reads `SYSTEM_MAIL_*` rather than `AUTH_*`. A separate verified domain is the intent, with a startup warning whenever system mail falls back to the marketing sender.

14. **Direction: system mail probably wants a separate *transport*, not just a separate domain.** A separate domain on the same provider still leaves one failure path — an ESP outage, suspension or quota locks operators out. The user's suggestion of the hosting provider's own SMTP (Strato) removes it, fits the volume, and costs nothing extra. Noted as direction with the design questions it opens (force it or not, self-hosting options, whether such a provider may be chosen for campaigns, how system emails are templated and triggered) logged as a Needs-ADR item — much of which turns out to be already implied by ADR-101/106/012/142.

**Status:** ADR-150–154 written as **proposed** and approved in review. The first slice is **built and verified**: users, roles and permissions as rows, `(user × role × brand)` assignment, passwordless code sign-in, revocable sessions, one route dependency, the access list with deactivation. Not built: passkeys, step-up authentication, SSO/SCIM, brands beyond the implicit default.

### 2026-08-12 — Omni-Channel Direction (idea captured, interview deferred)

15. **The product is not an email tool — it is content management and orchestration for any API-reachable channel.** Email alone is not enough; the named targets are a **letter** (via a letter shop), a **push notification**, and **paid social**. Not all of it at once and not all in the POC — the question is whether the *structure* absorbs new channels cheaply. **The market read is the commercial half of the argument:** most customers don't do omni-channel today, and the reason is that it is too complex in the typical SaaS platforms, not that they don't want it. That makes it the sharpest expression of **pillar 3** (off-the-shelf platforms don't fit the workflow), and the capability model means an adopter can connect a *local* letter shop or a regional messaging service — the long tail no SaaS vendor will ever integrate. For the Mittelstand segment that is a stronger differentiator than anything currently in the business briefing.

16. **Per-channel variants under a shared campaign — and the manager chooses which channels to prepare.** Not one neutral composition that each channel renders its own way: a social ad is genuinely not the same artifact as a newsletter, and forcing one composition to serve both yields lowest-common-denominator content. A campaign is **not** required to cover every channel — coverage is shown and warned about, never enforced, because forcing every channel is pillar 3's own failure mode (the platform imposing a workflow). Same idiom already chosen for system mail: *allow it, default safely, warn loudly*. **Superseded in part on 2026-08-07 (Cluster 1 Q4):** coverage is derived and purely descriptive, and there is **no warning** — because a *variant* is planned and sent, not a campaign, so there is no campaign-level send event at which to warn. Nobody asked to send push, so nothing is missing.

17. **Organic/broadcast social is out of scope; personalized paid social is in — and that decision simplifies the architecture rather than complicating it.** The interest is building a target group and handing it to Meta/TikTok for one-to-one delivery, not posting to everyone. This matters structurally: a broadcast post has **no recipient**, which would have forced `DeliveryExecutionDB.recipient_id` to become nullable and left campaign-level and per-recipient sends sharing an execution table with incompatible meanings. Paid social with custom audiences is still *addressed*, merely **delegated** — so the execution model survives intact. What remains is **three execution shapes**: addressed (platform decides who gets what), audience-delegated (platform supplies audience + creative, the ad platform decides delivery), and broadcast — struck out.

18. **Roughly 80% of the architecture is already channel-neutral, and that is a consequence of earlier decisions rather than luck.** [[ADR-012 — Content Records Represent Communication Units]] deliberately defines a record as a communication unit rather than an email; the decision layer resolves content records and knows nothing about email; the signal and audience layers are neutral in shape; and [[ADR-022 — Delivery Type Is Independent From Composition]] plus [[ADR-100 — Provider Layer as Send and Feedback Adapter]]/[[ADR-101 — Provider Capabilities Are Explicit]] already provide the seams. **This is worth stating in the playbook** — it is a concrete, checkable instance of the claim that ADR discipline pays off later, and a better proof than any argument about documentation quality in the abstract.

19. **Known limit to state honestly rather than discover: one-to-one does not generalize to every channel.** Ad platforms enforce minimum audience sizes, identifiers go one-way (hashed, never matched back), match rates are partial, and feedback arrives **aggregate per audience+creative**. So the long-term "smaller and smaller groups, down to one-to-one" direction holds for email, push, SMS and letter but **not** for paid social. Aggregate feedback must not be attributed down to individuals — doing so would corrupt the per-recipient signal layer, which is the thing that makes the decision layer trustworthy. The one real per-recipient path there is the adopter's *own* site via a conversion pixel/API.

20. **Timing: this is cheaper before the React frontend than after.** The product frontend is deliberately unstarted, so doing this first lets the manager UI be designed against channel-shaped variants rather than retrofitting a second codebase. It also overlaps the open **P0 consent bug** — per-channel consent restructures the same send-time gate the P0 fix must touch, so designing it first means fixing that once, in the right shape. Against that: it delays the hardening queue, and the interview is five clusters of real work.

**Status:** **idea captured, interview deferred by the user (2026-08-12), no code changes.** Written up as [[Omni-Channel - design interview]] — analysis, what is already settled, the five email-bound places in the code, and a planned five-cluster / 25-question interview expected to produce roughly a **160-block** of ADRs. Logged in `docs/backlog.md` as a Needs-ADR item. Note it also reopens naming: [[ADR-001 — Newsletter Architecture Boundaries]] scopes the core to "newsletter-specific responsibilities" and would need a superseding boundary ADR.
...
### Status at a glance — updated 2026-08-02
...
| — | **AI layer build — Mode A, first task.** `AIProvider` adapter + zero-cost mock · versioned manager-owned prompts · spend cap enforced as a pre-call gate · audit/cost ledger · "Suggest subject & preheader" on the variant with inline accept | ✅ done |
| — | **Real Claude adapter** — provider-counted tokens feeding the pre-call gate, governed model selection in Settings, USD spend alongside the token bar, free mock runs excluded from the ledger. Verified live on `claude-opus-5`. | ✅ done 2026-08-02 |
| — | Mode A **shared approval inbox** (ADR-141) — the suggestion-history surface; today only an inline accept on the variant exists | ❌ not built |
| — | **Second, EU-hosted model adapter** — ADR-144 §1 committed to *two* worked examples so "GDPR-friendly is possible" is demonstrated rather than claimed | ❌ not built |
...
| — | **AI-layer review** — `/interview-review` over the Mode-A build, 8/8 cleared; four act/fix logged, incl. the cap's concurrency race promoted to a general contention-model question | ✅ done 2026-08-02 |
| — | **Security design** — five ADRs: [[ADR-150 — Tenancy and Access Model]] · [[ADR-151 — Authentication and Sessions]] · [[ADR-152 — Secret and Credential Handling]] · [[ADR-153 — Audit and Accountability]] · [[ADR-154 — Erasure and Retention]] | ✅ designed 2026-08-02 (proposed, reviewed) |
| — | **Security base built** — users · roles/permissions as rows · `(user × role × brand)` · passwordless code sign-in · revocable sessions · one router-level guard over the whole UI · access list with deactivation · system-mail channel | ✅ done 2026-08-02 |
| — | Security: **enforcement flag** (mechanism covers every UI page; ships off until a deployment has signed in once) | 🟡 built, not switched on |
| — | Security: **passkeys** as an additive layer · **step-up auth** for destructive actions · SSO/SCIM · brand switcher | ❌ not built |
| — | **Technical onboarding manual** (Obsidian module/flow map in `docs/architecture/Code/` + FastAPI Swagger + code comments) | ✅ done |
| 4 | 4A structure · 4B client cases · 4C write & publish · 4D evolving-the-package / "AI-open" | 🟡 4A started (Cowork) |

**Emergent theme — Automation: design ✅ 2026-07-31, first build ✅ 2026-08-02.** The five-cluster AI-layer interview is closed and written up as **ADR-140–144** (Decision Log, three entries dated 2026-07-31). This unblocks engagement-driven audiences and send-time optimization. The "five ADRs deep and zero lines implemented" state it was in has been **fixed as intended**: Mode A shipped against a real Claude adapter, the design validated cheaply before anything expensive was built on top, and the `/interview-review` pass over it cleared 8/8.

**3D is unblocked but should NOT be started yet.** The Mode-A build produced a finding that changes it: per-recipient LLM *selection* is the wrong tool rather than merely an expensive one, and "AI" currently means two different things across ADR-080/081/082 (model-agnostic ranking) and ADR-140–144 (a language-model layer). Starting 3D before that is settled means building the expensive version by default, because that is the one the recent ADRs describe. Full cost model and reasoning in `docs/backlog.md` § Needs ADR.
...
### Suggested next steps — updated 2026-08-02
...
1. ~~**Build Mode A — first AI feature.**~~ ✅ **Done 2026-08-02.** Subject/preheader suggestion against the Claude API as the first real `AIProvider`; the design validated cheaply, as intended. The seed-content prerequisite is satisfied (42 real travel records imported; campaign 5 built on them — demo campaigns 1–4 remain filler). **Two pieces deliberately not built and now tracked in the status table above:** the shared approval inbox, and the second EU-hosted adapter that ADR-144 §1 committed to. **One finding from the build changed the AI roadmap** — see `docs/backlog.md` § Needs ADR: per-recipient LLM *selection* is the wrong tool, not merely expensive, so 3D should not be started before "what does AI mean in the decision layer" is settled (ADR-081's model-agnostic *ranking* vs ADR-140–144's LLM layer).

1b. **Security chapter — design** *(chosen next, 2026-08-02)*. Promoted from "parked" because it is **the only named blocker on 4C (write & publish)**, and because user/role authentication has the widest blast radius of anything left: it touches every route, so defining the standard *before* more surface is built is cheaper than retrofitting. Scope: authentication, roles/permissions, key storage, GDPR/ISO posture. **User's starting direction (2026-08-02, not yet a decision):** passwordless email login — a code or link — reusing what is planned for the library website, on the view that it may make SSO unnecessary. To be worked through as a design interview, one question at a time, before any code.
