...
- **Addendum 2026-09-18, and it closes a gap point 7 did not notice it had.** [[ADR-150 — Tenancy and Access Model]] §6 grants a human access as `(user × role × brand)` — permissions arrive *via a role*. Point 7 above states an integration's grants as `(principal × permission × brand)` — permissions *directly*. Point 7 presents itself as merely derived from point 1, which is how the two shapes came to differ without anyone deciding that they should. **Decided: an integration holds permissions directly, not through a role.** A role is a human convenience for naming a job, and an integration does not have a job — it has the list of calls it makes; modelling "n8n-sender" as a role would fill a company's role list with single-purpose entries that are really integrations wearing a hat. The cost is booked honestly: `permissions_for` resolves a user by walking `role_permissions → roles → role_assignments`, so a second resolution path now exists, and point 1's anxiety about duplication applies to it. The mitigation is that both paths must terminate in the same `has_permission` answer, and a test should assert that an integration and a user granted the same permission on the same brand are indistinguishable to every guard.
- **Addendum 2026-09-18:** [[ADR-152 — Secret and Credential Handling]] was accepted later the same day, which closes the last of the five proposed foundations. The Context above is left as written — it records what was true when this was accepted, a few hours earlier — and this note supersedes its final sentence rather than rewriting it. Nothing in the Decision depended on that record's status, only on its content, which did not change.
...
- **Built 2026-09-18, in four stages, with three decisions this record did not make.** They are recorded here rather than folded into the Decision above, which is fixed.
  1. **~~The JSON API takes a machine credential and not a session cookie.~~ REVERSED 2026-09-19 and built — see [[ADR-168 — The Manager SPA Authenticates With Its Session Cookie]].** This narrowing held for one day. It left the React manager client with no way to authenticate at all, which is what forced the question. `enforce_api_policy` now accepts either credential and refuses a request carrying both, and `enforce_api_csrf` defends the plane with a header-borne token — the form-borne one could not be reused, since it reads a JSON body as an empty `FormData` and is inert without a cookie. `policy.py` and `permissions_for` are untouched, so point 1 still has exactly one place answering "may this caller do this here". **What ADR-168 reuses from this record is point 8's brand asymmetry** — a person's working brand rides in the session, a machine states one per request — which is why the SPA is never asked for `X-Brand`. The original reasoning, kept because it was sound and was outlived rather than wrong:  Point 1 says a machine caller is a principal in the same access model; it does not say whether a *person* may use the JSON API, and the answer taken is no. That narrowing settles CSRF on this plane by construction — there is no ambient credential for a cross-site request to carry — and keeps `enforce_csrf`, which reads a request as a form, away from JSON bodies. A developer who wants to call the API issues themselves an integration key, which is attributable and revocable in a way a browser session is not.
  2. **`view` is not implied for an integration**, although every *role* implies it. For a person, "may edit and not read" is not a case worth modelling; for a machine it is the ordinary case — an orchestrator fires a send and reads nothing, a website form pins a recipient and reads nothing. Implying it would hand every integration the recipient list, which is one of the four routes the Context names as the reason this record exists.
  3. **Point 5 became a queue on 2026-09-19**, as the note below always said it would. The paragraph that follows records why it shipped as a refusal for one day, and is kept rather than rewritten because the reasoning is the point: a control that looks like a control and is not is worse than no control. What changed with it is the **order of the checks** in `enforce_api_policy`. The unattended-send flag was tested before `has_permission` and before the brand was resolved, on the argument that "you may not do this at all outranks you did not say where". That argument does not survive the flag becoming a queue — `ApprovalRequired` stopped being a "may not" and became a "not yet" that *hands out* a held request. Left in that order, an integration holding no `sends.execute` grant would have minted a pending send for a person to approve: privilege escalation through the approval surface. Both checks now precede it, and the brand does too, because a held request with a null brand is invisible in every brand's inbox.
