...
- **The concrete key list for point 2 is not enumerated here.** The decision is that the vocabulary is split finer and why; which keys `audiences.manage` becomes, and whether any other key splits with it, belongs to the [[ADR-150 — Tenancy and Access Model]] amendment alongside the code that guards them.
