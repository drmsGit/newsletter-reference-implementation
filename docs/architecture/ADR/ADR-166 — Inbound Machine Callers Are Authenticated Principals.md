...
- **Built 2026-09-18, in four stages, with three decisions this record did not make.** They are recorded here rather than folded into the Decision above, which is fixed.
  1. **The JSON API takes a machine credential and not a session cookie.** Point 1 says a machine caller is a principal in the same access model; it does not say whether a *person* may use the JSON API, and the answer taken is no. That narrowing settles CSRF on this plane by construction — there is no ambient credential for a cross-site request to carry — and keeps `enforce_csrf`, which reads a request as a form, away from JSON bodies. A developer who wants to call the API issues themselves an integration key, which is attributable and revocable in a way a browser session is not.
