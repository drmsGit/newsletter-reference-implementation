...
## Addendum 2026-09-15 — consent carries a brand

[[ADR-150 — Tenancy and Access Model]] was accepted 2026-09-14 with multi-brand as the **ordinary** case, and its brand columns were built the next day in `backend/scripts/migrate_0007_brand_scoping.sql`: `content_records`, `campaigns`, `audience_groups` and `send_instances` each carry a NOT NULL `brand_id`. **Consent deliberately did not** — that script says so in its own header, naming brand on the consent grid as Phase 2 work that needs an addendum to this record first, "because it changes a compliance path". This is that addendum. Nothing here changes the Decision section above; it widens the cell point 1 defines.

**The gap, verified rather than assumed.** `ConsentEventDB` (`backend/app/recipients/db_models.py:33-73`) is keyed `(recipient_id, channel, purpose)`, with `ix_consent_events_cell_latest` over `(recipient_id, channel, purpose, created_at)` — the index shaped for the latest-row-per-cell read. No brand is in the cell, and none is in any gate that reads it: `is_consenting_filter()` at `backend/app/audience/service.py:181` (the criteria gate) and `:439` (the consent floor in `resolve_audience`), the send-time consent stage at `backend/app/delivery/exclusion.py:137`, and the per-recipient refusal `require_consent` at `backend/app/decision/service.py:40`. Every one of them takes a channel and a purpose and nothing else. So the consequence is exact and it is live today: **a campaign scoped to brand B still reaches every consenting recipient**, because the authoring side is scoped and the send side is not. The code already admits it — `list_groups` (`backend/app/audience/service.py:23-29`) notes that groups "resolve to the same people whichever brand asks … consent does not carry one yet".

**1. The consent cell gains a brand: `(recipient, brand, channel, purpose)`.** Latest row wins within that cell, unchanged in shape. This is a wider key, not a second mechanism: append-only events, `source`, provability and fail-closed absence all hold exactly as point 1 states them.

**Consent to brand A says nothing about brand B.** That follows from the split rather than being an extra rule, and it is written down because the consequence is sharp: **a newly created brand starts with zero reachable recipients** until consent is captured for it. That is correct — it is the point — and the alternative is brand B's first campaign landing on people who agreed to hear from brand A. It will still surprise whoever creates their second brand and finds the audience empty, so it belongs on the record rather than in the discovery.

**2. An opt-out defaults to the brand that sent the mail.** The unsubscribe page additionally offers stopping everything from the company, and **whether that option appears is a company setting** — a one-brand deployment offering a choice between "this brand" and "all brands" is nonsense at best and alarming at worst, implying sibling brands the reader has never heard of.

Both sides were weighed, and both are real. Scoping the opt-out follows from scoping the consent, and a person genuinely subscribed to two brands should not lose both by leaving one. Against that: a person who meant *stop emailing me* must not keep hearing from the same company under another name. That is how complaints and spam reports start — and a complaint is precisely what this record's evidence trail exists to answer. The default covers the first case, the offered option covers the second, and the setting keeps the question from being asked where it has no meaning.

This governs a surface that does not exist yet — there is no unsubscribe page in the codebase (checked 2026-09-15). The only path that writes an opt-out today is the provider webhook (`backend/app/providers/service.py:186-218`), which already scopes it to the execution's `channel` and `purpose`. Brand derives the same way, from data already in hand: execution → send instance, and `send_instances.brand_id` has been NOT NULL since `migrate_0007`.

**3. The CRM carries the brand, and an assertion that names none is refused.** `sync_consent_from_crm` (`backend/app/recipients/service.py:252-261`) already takes `channel` and `purpose` as keyword arguments; brand joins them, **read from the CRM's own assertion** rather than configured per connection. [[ADR-120 — CRM as Customer Source of Truth]] makes the CRM the single source of truth for what a person permitted; which brand they subscribed to is the same kind of fact, not an exception to it.

**An assertion naming no brand is REFUSED, not defaulted to any brand.** Two precedents converge. This record's gates are fail-closed throughout — no event is no consent, and the SQL filter inherits that because `NULL = 'opted_in'` is never true (`backend/app/recipients/consent.py:203-237`). And `backend/scripts/migrate_0001a_consent_expand.sql:112-113` chose `source='migration'` over `source='crm'` for its backfill "because asserting the CRM said this would be inventing evidence". Landing a brandless flag on the default brand is that same invention, committed on the record that answers a UWG §7 complaint: it manufactures a grant for a brand nobody named. It costs nothing to refuse today — `consent_sync_logs` holds **0 rows** (dev database, 2026-09-15), so there is no integration to break. A fail-closed choice that is free now and expensive later is worth making now.

The refusal lands on a surface that is currently narrower than the function behind it: `ConsentSyncRequest` (`backend/app/recipients/models.py:39-44`) carries `consent_status`, `source` and `note` — not channel, not purpose, and so not brand. That payload has to grow all three before it can be refused for lacking one.

The refusal stands because [[ADR-120 — CRM as Customer Source of Truth]] makes the CRM the source of truth for what a person permitted, and a company that has brands has the brand on hand to send. **A single-brand company is not blocked by it** — its CRM has no brand field to send precisely because there is only one brand, so it hardcodes the brand in the API call: one constant in an integration it is writing anyway. That removes the only real objection to failing closed here, because the refusal is then not a burden placed on the smallest adopters — for them the compliance property costs a literal.

**4. The 41 existing `consent_events` rows belong to the default brand.** All 41 are `(email, marketing)` with `source='migration'`, written 2026-07-25 to 2026-07-27, and the default brand is the only brand that existed until 2026-09-15. This is *not* the invention point 3 refuses: there was exactly one brand, so that is the only value those rows could ever have had — the same distinction `migrate_0007` drew for its own backfill, between a guess standing in for unknowable history and the only available answer. One difference matters and is recorded here rather than left to be hit: `migrate_0007` established that precondition by counting brands and refusing above one, and **a second brand now exists** (created 2026-09-15). Whatever assigns a brand to these rows therefore has to assert something about *the rows* — that each predates every brand but the default — not about how many brands the database currently holds. What must be true is stated; the sequencing is not designed here.

**Blocked on this work: the category graph's brand filter.** [[ADR-150 — Tenancy and Access Model]]'s second 2026-09-15 addendum defines a brand's audience as everyone who has consented to that brand, which is inexpressible until this change exists; that record notes the dependency from its side, and this is the other end of it.

**The costs, stated plainly.** This requires the adopter's CRM to model brands the way this platform does — the kind of assumption about somebody else's system that [[ADR-126 — Maintain Local Recipient Projection]]'s local projection exists to avoid, and a widening of what Decision point 5 already demands of a single-flag CRM: not one grid now, but one per brand. It was chosen anyway, because the alternative silently manufactures consent, and there is no third option that is both quiet and honest. The rest follows from the wider key. Every read path gains a dimension it must be *passed* rather than default, since a defaulted brand inside a gate is exactly the fail-open this addendum refuses at the CRM boundary. Every write path has to name one — the `source="import"` event in `create_recipient`, the CRM sync, the provider suppression. And a person who hears from three brands now carries three cells per `(channel, purpose)` instead of one, which is the ordinary price of the grid and the same price the channel dimension already charges.
