...
- **Signature verify fails closed** — with no `RESEND_WEBHOOK_SECRET` there is nothing to verify against, so every webhook is rejected (401) and an error logged; a dev bypass would have to be a deliberate flag, never the absence of config. Note the secret is read at startup, so it needs a full restart to take effect.
