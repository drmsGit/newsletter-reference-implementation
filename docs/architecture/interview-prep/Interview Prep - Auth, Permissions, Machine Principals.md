...
- [x] **Q2.** Permissions are grants only, with no DENY, and multiple roles union. Why not model denies, and where does the union get dangerous?
    ...
    **Resolution (2026-09-19): act/fix, and filed as the same defect as [[Interview Prep - Brand Boundary]] Q2 rather than a separate one.** The union is correct and needs no change — it falls out of grants-only, and two roles cannot contradict each other because there is no DENY to resolve. What gets fixed is the **overloaded optional**: `brand_id=None` means "every brand this principal holds", so the safe reading and the dangerous one are the same call. `_permitted` already refuses rather than passing `None` through, which is the right guard and is tested — but it is a guard against an argument shape that should not be expressible. A typed scope makes the union something a caller has to name.

- [x] **Q3.** Every role implies `view`, but an integration is not granted it. Defend the asymmetry — and what does it do to machine GETs?
    ...
    **Resolution (2026-09-19): act/fix.** The asymmetry stays — write-without-read is the ordinary machine case, and implying `view` would hand every integration the recipient list, which is one of the four exposures [[ADR-166 — Inbound Machine Callers Are Authenticated Principals]] exists to close. What is wrong is the **fall-through**: every GET on the machine plane resolves to `VIEW`, so an integration granted `recipients.manage` can write a contact and is refused reading one back, and the only available remedy is a platform-level grant that reopens the exposure. A CRM connector that cannot read back what it wrote cannot reconcile, so this becomes real as soon as the CRM is scoped. **Raised in weight by [[ADR-168 — The Manager SPA Authenticates With Its Session Cookie]]**, which put people on this plane too: `view` now carries two meanings on one set of routes.
