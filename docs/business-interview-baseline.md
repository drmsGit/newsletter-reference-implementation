...
- **E1** — Human-governed taxonomy before AI ([[ADR-080 — Human-governed Taxonomy Before AI Selection]]) — feeds Phase 3D (AI-based audience selection), which itself depends on 3C. Two phases out.
- **E3** — "Email must not be sent if no meaningful content remains" ([[ADR-086 — Decision Slots Fail Gracefully]]) — no sendability check exists in `send_send_instance` today; the ADR's mandate has no corresponding feature yet.
- **F1** — Consent/eligibility gap ([[ADR-004 — Privacy Operations as a First-Class Architectural Concern]], [[ADR-122 — Minimal Consent Model Required]]) — already noted in [[MOC - Interview Prep Baseline]] as an accepted POC-scope gap; no consent feature is queued.
- **F2** — Bounce/complaint-as-mandatory, deliverability assumption ([[ADR-106 — Bounce and Complaint Feedback Is Mandatory]]) — no real provider adapter exists or is scheduled; only `MockProvider`.
- **H1** — Provider independence unproven ([[ADR-100 — Provider Layer as Send and Feedback Adapter]] / 101 / 105) — same reason; the abstraction has never been exercised against a real vendor.
...
### E1. [[ADR-080 — Human-governed Taxonomy Before AI Selection]] / [[ADR-081 — AI Ranks Within Governed Candidate Sets]] — AI never chooses outside a human-built taxonomy `🟡 deferred`
...
### E3. [[ADR-086 — Decision Slots Fail Gracefully]] — "If no meaningful newsletter content remains, the email must not be sent" `🟡 deferred`
...
### F1. [[ADR-004 — Privacy Operations as a First-Class Architectural Concern]] and [[ADR-122 — Minimal Consent Model Required]] vs. the actual `recipients` implementation `🟡 deferred`
...
### F2. [[ADR-106 — Bounce and Complaint Feedback Is Mandatory]] — deliverability/reputation as a universal concern `🟡 deferred`
...
### H1. [[ADR-100 — Provider Layer as Send and Feedback Adapter]] / [[ADR-101 — Provider Capabilities Are Explicit]] / [[ADR-105 — Provider-Specific Data Must Not Be Architecture-Critical]] — provider independence and swap-ability `🟡 deferred`
...
## M. New Concepts Surfaced (2026-07-05)

Not resolutions of existing findings — two new ideas that came out of the B2/E2/J1 discussion, worth tracking as their own concepts rather than folding into an existing mechanism. Logged to `docs/backlog.md` for future scoping.
...
- `docs/playbook-strategy.md` — the actual business/product strategy source most of these findings trace back to
